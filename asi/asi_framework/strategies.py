...
means writing its run function + resumable-state dataclass (see search.py's
...
class StrategySpec:
    ...
    state_cls: type            # exposes .load(outputdir) / .STRATEGY, for resume-mismatch diagnostics
...
STRATEGIES: dict[str, StrategySpec] = {
    "greedy": StrategySpec(
        ...
        state_cls=greedy.GreedySearchState,
    ),
    "spea2": StrategySpec(
        name="spea2",
        run=spea2.explore_pareto_front_spea2,
        state_cls=spea2.Spea2SearchState,
    ),
}
